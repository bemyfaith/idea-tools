...
function App() {
  ...
  const [libraryAnimDuration, setLibraryAnimDuration] = useState(3)
  ...
  const makeLibraryItem = (fileName: string, src: string, posIndex: number) => {
    ...
    return createCanvasItem({ name: fileName, src, x: pos.x, y: pos.y, width: 180, height: 180, rotation: 0, categoryId: '', sourceId: uid(), isPreview: false, animationDuration: libraryAnimDuration })
  }
  ...
  const previewLibraryItem = (item: CanvasItem) => {
    ...
    const nextItem = createCanvasItem({
      ...
      isPreview: true,
      animationDuration: libraryAnimDuration,
    }, 'searching')
    setTemplateState((prev) => ({
      ...prev,
