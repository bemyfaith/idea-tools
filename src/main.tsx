...
function App() {
  ...
  const handleRailPointerDown = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    const onMove = (ev: PointerEvent) => {
      if (!draggingRailRef.current || !stageRef.current) return
      const stageBox = stageRef.current.getBoundingClientRect()
      const next = Math.max(92, Math.min(ev.clientX - stageBox.left, Math.min(420, stageBox.width - 120)))
      setRankRailWidth(next)
    }
    const onUp = () => {
