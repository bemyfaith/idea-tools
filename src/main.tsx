...
function App() {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      {contextMenu && (() => { const current = library.find((item) => item.sourceId === contextMenu.sourceId)?.animationDuration ?? 3; const isActive = (v: number) => Math.abs(current - v) < 1e-9; return <div className="context-menu" style={{ left: contextMenu.x, top: contextMenu.y }} onClick={(e) => e.stopPropagation()}><button className={isActive(0.5) ? 'active' : ''} onClick={() => applyLibraryDuration(contextMenu.sourceId, 0.5)}>0.5 秒</button><button className={isActive(1) ? 'active' : ''} onClick={() => applyLibraryDuration(contextMenu.sourceId, 1)}>1 秒</button><button className={isActive(2) ? 'active' : ''} onClick={() => applyLibraryDuration(contextMenu.sourceId, 2)}>2 秒</button><button className={isActive(3) ? 'active' : ''} onClick={() => applyLibraryDuration(contextMenu.sourceId, 3)}>3 秒</button><button className={isActive(5) ? 'active' : ''} onClick={() => applyLibraryDuration(contextMenu.sourceId, 5)}>5 秒</button><button className={isActive(8) ? 'active' : ''} onClick={() => applyLibraryDuration(contextMenu.sourceId, 8)}>8 秒</button></div> })()}
    </aside>
