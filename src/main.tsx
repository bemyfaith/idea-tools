...
function App() {
  ...
  const [rankRailWidth, setRankRailWidth] = useState(141)
  ...
  return <div className="app-shell">
    ...
    <aside className="sidebar">
      ...
      {selected && <div className="panel selected-panel"><div className="panel-title">当前选中图片</div><div className="selected-panel-row"><div className="selected-panel-label">等级</div><select value={selected.categoryId || ''} onChange={(e) => moveSelectedToCategory(selected.id, e.target.value)}><option value="">无</option>{rankCategories.map((category) => <option key={category.id} value={category.id}>{category.label}</option>)}</select></div><div className="selected-panel-actions"><button className="danger" onClick={() => removeItem(selected.id)}><Trash2 size={16} />删除</button></div></div>}
      {templateId === 'clean' && <div className="panel"><div className="panel-title">动画编辑</div><div className="settings-stack"><div className="setting-row"><label className="setting-label"><input type="checkbox" checked={deltaSearchEnabled} onChange={(e) => setDeltaSearchEnabled(e.target.checked)} />启用🔍动画</label></div><div className="setting-row"><div className="setting-label">默认时长（秒）</div><div className="setting-inline"><input className="rank-size-input" type="number" min="0.5" step="0.1" value={deltaSearchDefaultDuration} onChange={(e) => setDeltaSearchDefaultDuration(e.target.value)} /></div></div><div className="setting-row"><div className="setting-label">红金音效</div><div className="setting-inline"><span>≥</span><input className="rank-size-input" type="number" min="0.5" step="0.1" value={soundSettings.redgoldMin} onChange={(e) => setSoundSettings((prev) => ({ ...prev, redgoldMin: e.target.value }))} /></div></div><div className="setting-row"><div className="setting-label">紫蓝音效</div><div className="setting-inline"><input className="rank-size-input" type="number" min="0.1" step="0.1" value={soundSettings.greenMax} onChange={(e) => setSoundSettings((prev) => ({ ...prev, greenMax: e.target.value }))} /><span>-</span><input className="rank-size-input" type="number" min="0.1" step="0.1" value={soundSettings.redgoldMin} onChange={(e) => setSoundSettings((prev) => ({ ...prev, redgoldMin: e.target.value }))} /></div></div><div className="setting-row"><div className="setting-label">绿色音效</div><div className="setting-inline"><span>≤</span><input className="rank-size-input" type="number" min="0.1" step="0.1" value={soundSettings.greenMax} onChange={(e) => setSoundSettings((prev) => ({ ...prev, greenMax: e.target.value }))} /></div></div></div></div>}
      <div className="panel"><div className="panel-title">等级模版编辑</div>
        <div className="canvas-edit-row"><label className="toggle-row"><input type="checkbox" checked={template.showRightDividers} onChange={(e) => setTemplates((prev) => prev.map((item) => item.id === templateId ? { ...item, showRightDividers: e.target.checked } : item))} />分割线</label><div className="canvas-background-wrap"><div className="canvas-background-label">画布背景色</div><input className="canvas-background-input canvas-background-input-round" type="color" value={canvasBackgroundColor} onChange={(e) => setCanvasBackgroundColor(e.target.value)} /></div></div><div className="canvas-edit-row"><div className="canvas-background-label">等级宽度设置</div><div className="setting-inline"><input className="rank-size-input" type="number" min="92" max="420" step="1" value={rankRailWidth} onChange={(e) => setRankRailWidth(Number(e.target.value) || 141)} /><span>px</span></div></div>
